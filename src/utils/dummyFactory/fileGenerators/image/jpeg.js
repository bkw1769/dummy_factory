/**
 * JPEG 이미지 파일 생성 함수
 * Canvas API를 사용하여 실제 유효한 JPEG 파일 생성
 */
 ...
 * @returns {Promise<Blob>} 생성된 JPEG Blob
 */
export const generateJPEG = async (sizeMB, unit = "auto") => {
  const byteSize = convertMBToBytes(sizeMB, unit);

  // 최소 JPEG 크기
  const minJPEGSize = 100;
  if (byteSize < minJPEGSize) {
    // 최소 크기보다 작으면 1x1 픽셀 JPEG 생성
  ...
  // 이미지 크기 계산
  // JPEG는 압축되므로 정확한 크기 예측이 어렵습니다
  // 대략적인 픽셀 수 계산 (RGB, 3 bytes per pixel + 압축률 고려)
  // JPEG 압축률을 10:1로 가정
  const estimatedPixels = Math.floor((byteSize * 10) / 3);
      ...
      canvas.toBlob(
        (blob) => {
          if (!blob) {
            ...
            resolve(new Blob([fallback], { type: "image/jpeg" }));
            return;
          }

          // 크기가 부족하면 추가 데이터를 JPEG 뒤에 추가
          if (blob.size < byteSize) {
            ...
            blob.arrayBuffer().then((buffer) => {
              ...
              resolve(new Blob([combined], { type: "image/jpeg" }));
            });
