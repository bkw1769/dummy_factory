/**
 * PNG 이미지 파일 생성 함수
 * Canvas API를 사용하여 실제 유효한 PNG 파일 생성
 */
 ...
 * @returns {Promise<Blob>} 생성된 PNG Blob
 */
export const generatePNG = async (sizeMB, unit = "auto") => {
  const byteSize = convertMBToBytes(sizeMB, unit);

  // 최소 PNG 크기 (헤더 + 최소 이미지 데이터)
  const minPNGSize = 100;
  if (byteSize < minPNGSize) {
    // 최소 크기보다 작으면 1x1 픽셀 PNG 생성
      ...
      (blob) => {
        ...
        if (blob.size < byteSize) {
          ...
          blob.arrayBuffer().then((buffer) => {
            ...
            const combined = new Uint8Array(buffer.byteLength + additionalBytes);
